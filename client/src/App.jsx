import React from 'react'
import {Link,Route,BrowserRouter,Routes} from 'react-router-dom'
import { logo} from './assets'
import { Home,CreatePostt } from './pages'

const App = () => {
  
  return (
    <BrowserRouter>
        <header className='w-full flex justify-between items-center bg-white
        sm:px-8 px-4 py-4 border-b-[#e6ebf4]'>

            <Link to="/">
              <img src={logo} alt="logo" className='w-28 object-contain' />
            </Link>

            <Link to="/create-post" className='font-inter font-medium bg-[#6469ff]
             text-white px-4 py-2 rounded-md'>
              Create
            </Link>
        </header>

         {/* here we are calculating the height that we need for the main container
         ie, we are subtracting the full height minus the height of the nav bar */}
        <main className='sm:p-8 px-4 py-8 w-full bg-[#f9fafe]
        min-h-[calc(100vh-73px)]'>

          {/* inside this we want to render our routes */}

          <Routes>
            <Route path='/' element={<Home/>}/>
            <Route path='/create-post' element={<CreatePostt/>}/>

          </Routes>


        </main>
    
    </BrowserRouter>
  )
}

export default App